...
class Donor extends Component {
  ...
  fetchDonors = async () => {
    try {
      ...
      const response = await axios.get("http://localhost:8080/donors", { headers });
...
fetchTotalDonors = async () => {
    try {
      ...
      const response = await axios.get("http://localhost:8080/totalDonors", { headers });
  ...
  renderRecentDonors = () => {
    const recentDonors = this.state.donors
      .sort((a, b) => b.donorId - a.donorId) // Sort donors by donorId in descending order
