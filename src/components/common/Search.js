...
class Search extends Component {
  ...
  searchMyData = (event) => {
    ...
    axios
      .get("/search", { headers, params: { keyword: keyword } })
      .then((res) => {
      ...
      result = this.state.displaysearchdata.map((meal, key) => (
        <div className="col-sm-3" key={key}>
          <div className="card" style={{ width: "18rem", margin: "10px" }}>
            <img src={meal.mealImgPath} className="card-img-top" alt={meal.mealPhoto} style={{ height: "180px" }} />
            <div className="card-body">
              <h5 className="card-title mealname">{meal.mealName}</h5>
              <p className="card-text mealdesc">{meal.mealDesc}</p>
              <a href={`/mealdetails/${meal.meal_id}`} className="btn btn-warning">
                View Meal Details
              </a>
            </div>
          </div>
